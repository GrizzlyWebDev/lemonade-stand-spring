...
public class OrderServiceImpl implements OrderService {
    ...
    private void setUpOrder(Order order) {
        ...
        for (Lemonade lemonade : order.getLemonades()) {
            lemonade = lemonadeService.getLemonadeFromRequest(lemonade.getId());
            lemonades.add(lemonade);
            total += lemonade.getPrice();
        }
        order.setTotal(total);
        order.setLemonades(lemonades);
        order.setCustomer(customerService.getCustomerFromRequest(order.getCustomer().getId()));
        order.setLemonadeStand(lemonadeStandService.getLemonadeStandFromRequest(order.getLemonadeStand().getId()));
    }
